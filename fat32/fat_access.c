			...
			for (item = dirls->offset; item < FAT_DIR_ENTRIES_PER_SECTOR; item++)
			{
				...
				if ( fatfs_entry_sfn_only(directoryEntry) )
				{
                    ...
                    */
					...
					// Copy name to short_filename
                    filename_ptr = short_filename;
                    for (last_pos=7; last_pos>0; last_pos--)
                        if (directoryEntry->Name[last_pos] != ' ')
                            break;
                    for (i=0; i<=last_pos; i++)
						*filename_ptr++ = tolower(directoryEntry->Name[i]);

					// Copy extension (if any) to short_filename
                    for (last_pos=10; last_pos>7; last_pos--)
                        if (directoryEntry->Name[last_pos] != ' ')
                            break;
                    if (last_pos>7)
                    {
                        *filename_ptr++ = '.';
                        for (i=8; i<=last_pos; i++)
    						*filename_ptr++ = tolower(directoryEntry->Name[i]);
                    }
